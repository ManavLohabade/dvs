...
router.post('/data', async (req, res) => {
  try {
    ...
    if (existingTiming.rows.length === 0) {
      ...
      timingId = timingResult.rows[0].id;
      ...
      timingId = existingTiming.rows[0].id;
      console.log('Good timing already exists for today, ID:', timingId);
    }
    ...
    res.json({
      ...
      data: {
        ...
        day: todayDay,
        timingId: timingId,
        hasDaylight: existingDaylight.rows.length > 0
      }
    });
